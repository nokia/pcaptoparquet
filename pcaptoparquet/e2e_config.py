...
def load_mapping_from_file(configpath: Optional[str] = None) -> dict[str, Any]:
    """
    Loads a mapping from the specified file.

    Args:
        configpath (str): The path to the file containing the mapping.

    Returns:
        dict[str, Any]: The loaded mapping.
    """
    ...
    if configpath is None:
        return transportport_to_protocol
    ...
    with open(configpath, "r", encoding="utf-8") as file:
        ...
        for protocol in mapping_with_modules["protocols"]:
            ...
            if not os.path.isfile(protocol["module_path"]):
                raise FileNotFoundError(f"File {protocol['module_path']} not found.")

            decode_cb = load_function_from_file(protocol["module_path"], "decode")

            load_transport_protocol(
                transportport_to_protocol, protocol["transport_protocols"], decode_cb
            )

        # Existing applications (overrides)
        if "overrides" not in mapping_with_modules:
            return transportport_to_protocol
        ...
        for app in applications:
            if app in mapping_with_modules["overrides"]:
                ...
                if not protocol["module_path"] or not os.path.isfile(
                    ...
                    decode_cb = load_function_from_file(
        ...
        if callbacks is None:
            ...
            if callbackpath:
                ...
                for callback in callbacklist:
                    callbacks.append(load_function_from_file(callback))
