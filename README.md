...
- `run(capture, preset=…)` — mix/SNI/samples and tshark-style aggregations: `summarize_capture`, `list_flows`, `sni_table`, `filter_packets`, `tcp_setup`, `tcp_anomalies`, `app_messages`, `quic_initials`, `endpoints`, `conversations`, `io_stat`. `bytes` columns are `sum(ip_len)` when present. `summarize_capture` is a `section`/`key`/`value` table: packet count, duration, unique-IP counts, transport/app/encap/IP-version/DSCP mix, `tunnel_type` (not raw tunnel strings, not per-IP mix), flag/frag/ESP/SNI counts, and well-known **ports** (`udp/2123`, `tcp/80`, …). `tcp_anomalies` uses the same table shape for unmatched SYN 4-tuples and duplicate TCP seq groups. `io_stat` buckets are chronological; `group_limit` drops later time (`truncated`). Top destination ports and counted `app_request` strings are a one-step `plan`, not extra presets:
